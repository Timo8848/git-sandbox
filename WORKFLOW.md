...
适用人数：3 人小团队，代码主要由 AI（Claude Code）编写和合并。每个功能/修复走独立分支，AI 确认代码没问题后即可合并进 main。

---

## 核心原则

- **main 永远是可以运行的稳定版本**，不直接往 main 上写代码，所有改动走分支
- **一个分支只做一件事**，做完就合并，不攒着
- **改代码前先同步 main**，确认别人有没有新改动，避免冲突（这是最关键的纪律）
- **AI 确认代码没问题就可以直接 commit / merge**，不强制人工 approve；拿不准时再让人看一眼
...
AI 确认代码没问题后，直接点 **"Merge pull request"**，再点 **"Delete branch"** 删掉已合并的分支。如果改动比较大、或者拿不准，可以让一个同事先看一眼再合。

> 让 Claude Code 做的话，开 PR 和 merge 都可以一句话交给它，例如：「确认测试通过后帮我 merge 这个 PR 并删掉分支」。
